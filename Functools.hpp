...
namespace Functools
{
	...
	template<class T, class F>
	T map(const T& seq, const std::function<const F&>& lambda)
	{
		//iterate over the passed sequence and apply 
		//passed lambda to each element
		//then returns new sequence
	...
	namespace MapFP
	{
		...
		template<class T1, class T2, class F>
		std::map<T1, T2> map(const std::map<T1, T2>& hash_table, std::function<const F&>& lambda)
		{
			//map function for hash tables
	...
	template<class T, class F>
	T filter(const T& seq, const std::function<bool(const F&)>& predicat)
	{
		//if sequance elements matches to predicat, then save it
	...
	namespace MapFP
	{
		...
		template<class T1, class T2, class F>
		std::map<T1, T2> filter(const std::map<T1, T2>& hash_table, std::function<bool(const F&)>& predicat)
		{
			//filter function for hash tables
	...
	template<class T, class F, class A>
	A reduce(const T& seq, const std::function<F(const A&, const A&)>& lambda)
	{
		//this function was inspired by python's standart library
